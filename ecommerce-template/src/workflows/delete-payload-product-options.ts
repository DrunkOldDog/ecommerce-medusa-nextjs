    ...
    const updateData = transform(
      ...
      (data) => {
        const items = data.payloadProducts.map((payloadProducts) => ({
          ...
          options: payloadProducts.options.filter(
            (o: any) => !data.option_ids.includes(o.medusa_id)
          ),
          variants: payloadProducts.variants.map((variant: any) => ({
            ...variant,
            option_values: variant.option_values.filter(
              (ov: any) => !data.option_ids.includes(ov.medusa_option_id)
            ),
          })),
        }));
