...
export const createPayloadProductOptionsWorkflow = createWorkflow(
  ...
  ({ option_ids }: WorkflowInput) => {
    const { data: productOptions } = useQueryGraphStep({
      ...
      fields: ["id", "title", "product.payload_product.*"],
      filters: {
        id: option_ids,
      },
      options: {
    ...
    const updateData = transform(
      ...
      (data) => {
        ...
        data.productOptions.forEach((option) => {
          const payloadProduct = // @ts-expect-error needed for extending product type
          ...
          const existingOptionIndex = items[
            payloadProduct.id
          ].options.findIndex((o: any) => o.medusa_id === option.id);
