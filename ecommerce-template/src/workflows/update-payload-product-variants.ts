      ...
      (data) => {
        ...
        data.productVariants.forEach((variant) => {
          const payloadProduct = // @ts-expect-error
            variant.product?.payload_product as PayloadCollectionItem;
          if (!payloadProduct) {
            return;
          }
          ...
          const existingVariantIndex = items[
            payloadProduct.id
          ].variants.findIndex((v: any) => v.medusa_id === variant.id);
          ...
          if (existingVariantIndex >= 0) {
            ...
            const updatedOptionValues = variant.options.map((option) => ({
              ...
              value:
                existingVariant.option_values.find(
                  (ov: any) => ov.medusa_id === option.id
                )?.value || option.value,
            }));
